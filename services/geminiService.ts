...
import { NutritionData, User, WorkoutPlan } from "../types";
...
export const generateWorkoutPlan = async (user: User): Promise<WorkoutPlan> => {
  try {
    const prompt = `
      Crie um plano de treino semanal para uma pessoa com o seguinte perfil:
      - Objetivo: ${user.workoutGoal || 'Geral'}
      - Nível de experiência: ${user.experienceLevel || 'Iniciante'}
      - Frequência: ${user.daysPerWeek || 3} dias por semana
      - Foco: Hipertrofia e Saúde
      
      O plano deve ser detalhado, com exercícios, séries, repetições e tempo de descanso.
      Responda estritamente com o JSON solicitado seguindo o schema.
    `;
