...
const HistoryScreen: React.FC<HistoryScreenProps> = ({ history, onBack, onSelect }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 pb-32 no-scrollbar">
      <div className="bg-zinc-950/80 backdrop-blur-md p-4 sticky top-0 z-20 flex items-center justify-between border-b border-zinc-900">
        <div className="flex items-center gap-2">
            <button onClick={onBack} className="w-10 h-10 rounded-full hover:bg-zinc-900 flex items-center justify-center text-zinc-100 transition-colors">
            <ChevronLeftIcon className="w-6 h-6" />
            </button>
            <h2 className="text-xl font-bold text-white tracking-tight">Histórico</h2>
        </div>
      </div>
      ...
      <div className="p-4 flex-1">
        {sortedHistory.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-[60vh] text-center opacity-60">
            ...
            <h3 className="text-lg font-bold text-white mb-1">Nada por aqui</h3>
              ...
              <div key={date}>
                ...
                <div className="space-y-3">
                    ...
                    <div
                        ...
                        className="bg-zinc-900 p-2 pr-4 rounded-2xl shadow-sm border border-zinc-800 flex items-center gap-4 active:scale-[0.98] transition-transform cursor-pointer"
                    >
                        <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-zinc-800 relative">
                           <img 
                                src={item.imageUrl} 
                                alt={item.foodName} 
                        ...
                        <div className="flex-1 min-w-0 py-1">
                            <h4 className="font-bold text-white text-lg truncate mb-1">{item.foodName}</h4>
                            <div className="flex items-center gap-3">
                                <span className="text-sm font-semibold text-zinc-300">{Math.round(item.calories)} kcal</span>
                            ...
                            <div className="flex gap-1 mt-2">
                                ...
                                <div className="h-1 flex-1 bg-amber-900/30 rounded-full overflow-hidden">
